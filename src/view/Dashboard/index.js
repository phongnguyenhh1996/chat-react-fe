...
const Dashboard = () => {
  ...
  const handleOk = async () => {
    if (!MainStore.online) {
      ...
      MainStore.setCameraKey("player--" + randomPlayerId);
  ...
  const cameraKey = MainStore.cameraKey;

  useEffect(() => {
    let key = cameraKey.split("__")[0] || cameraKey;
    if (key === "reset") {
      MainStore.cameraRef?.resetTransform(200);
    } else {
      MainStore.cameraRef?.zoomToElement(key, 1.45);
    }
  }, [cameraKey]);
  ...
  const goNextAvailablePlayer = () => {
    ...
    MainStore.updateGameState(GAME_STATES.ROLL_DICE);
    MainStore.setCameraKey("player--" + MainStore.players[nextPlayerIndex].id);
  ...
  const movingPlayer = async (callback, planeDestinationPostion) => {
    ...
    if (planeDestinationPostion) {
      MainStore.updatePlayerData(currentPlayer, "position", newPosition);
      await delay(1000)
      MainStore.setCameraKey("player--" + currentPlayer.id);
      MainStore.sendDataToChannel(["players", "gameState"]);
    } else {
  ...
  return (
    <>
      <TransformWrapper
        ...
        onInit={(ref) => MainStore.setCameraRef(ref)}
      >
        <TransformComponent wrapperStyle={{ width: "100vw", height: "100vh" }}>
          <div
            className="container-page"
            style={{
              gridAutoRows: `minmax(${parseInt(
          ...
          >
                  ...
                  >
                    ...
                    >
                      <img
                        style={{
                          flex:
                            window.innerWidth > 950 ? "0 0 25px" : "0 0 15px",
            ...
            >
              {MainStore.cameraKey.startsWith("reset") && (
                <>
                  <div
                    style={{
                      position: "absolute",
                  ...
                  {MainStore.online && window.innerWidth <= 950 && (
                    <Popover
                      content={<form onSubmit={sendChat}>{chatInput}</form>}
                    >
                      <Button
                  ...
                  {(!MainStore.online ||
                    ...
                    <Popconfirm
                      ...
                      onConfirm={
                        !MainStore.online ? MainStore.resetGame : surrender
                      }
                      ...
                      >
                        ...
                        {MainStore.gameState.startsWith(
                          ...
                          MainStore.gameState.split("--")[2] !== "bank" && (
                            <div>
                              Mất {MainStore.gameState.split("--")[1]}$ khi đi
                              vào ô này
                            </div>
                          )}
                        {MainStore.gameState.startsWith(
                        ...
                        {MainStore.gameState ===
                          GAME_STATES.MAX_LEVEL_PROPERTY &&
                        ...
                        {MainStore.gameState ===
                          GAME_STATES.CURRENT_LOST_ELECTRIC &&
                          "Không mất tiền vì ô hiện tại đang mất điện"}
                        {MainStore.gameState ===
                          GAME_STATES.FREE_OUT_FAIL_CARD &&
                      ...
                      <div className="information__row">
                        {MainStore.gameState === GAME_STATES.WAITING && (
                          <>
                            ...
                            {range(0, MainStore.totalPlayers).map(
                              (idx, index) => (
                                <PlayerInfor
                                  key={
                                    (MainStore.players[idx]?.id || "noname-") +
                                    index
                                  }
                                ...
                                >
                                  ...
                                  <Popover
                                    ...
                                    content={
                                      <div style={{ color: COLORS[index] }}>
                                        {MainStore.chat[player.id]
                                          ? MainStore.chat[player.id].split(
                                              "--"
                                            )[0]
                                          : ""}
                                      </div>
                                    ...
                                    open={
                                      MainStore.chat[player.id] &&
                                      moment(
                                        MainStore.chat[player.id].split("--")[1]
                                      )
                                        .add("5", "second")
                                        .isAfter(moment()) &&
                                      (player.broke ||
